...
| E3 | 低カバレッジ4文 | 4 | 音素カバレッジが低い対照群（目標20-25音素、約50-65%） |
...
#### 目的
100文から37音素インベントリを抽出し、4条件のコーパス（80文/37音素4文/低カバレッジ4文/上位10文）の文IDリストを生成する。
...
- [ ] 低カバレッジ4文の選定（ユニーク音素数が少ない文を優先、目標20-25音素）（JVSデータ準備後に実行）
...
#### 変更点
- ユニーク音素数が少ない4文を優先的に選定（対照群）
- 音素カバレッジを意図的に低くする（目標20-25音素、約50-65%）
- 選定方法: 各文のユニーク音素数でソートし、音素数が少ない文を優先選択

#### 期待される結果
- 4条件中で最も低い品質（MCD高、F0 RMSE高、CER高）
- 音素カバレッジが低い場合の品質低下を明確に示す
- E2（高カバレッジ、100%）との明確な対比により、カバレッジの重要性を検証

#### 採用基準
- 目標カバレッジ: 20-25音素（38音素中、約50-65%）
- シード固定により再現可能
- 含有音素数を計測・記録
- 学習が正常に収束（品質は低い可能性）
...
| Fine-tuning時間超過 | 高 | ステップ数削減、条件数削減（E3（低カバレッジ4文）を除外） |
